	...
	var miciudad; // Ciudad actual con la que estoy buscando una direccion

var tituloinfo = "Informaci贸n";
...
function buscar(tipoDireccion) {
	...
	if ((calle != "" && altura !="") && (isNaN(altura)==false)) {
		...
		jAlert("Debe ingresar una direcci&oacute;n correcta.","Errores de Geolocalizaci贸n");
	}
}
    ...
    {  
        // Propiedades de la georreferenciaci髇  
        var request = {  
            address: location  
        }  
        ...
        // Invocaci髇 a la georreferenciaci髇 (proceso as韓crono)  
	...
	function direccionGoogle(data,tipoDireccion) {
		...
		//		" <a href=http://maps.google.com/maps?t=m&q="+lat+","+ lng+" target='_blank'>Comprobar Coordenadas!</a>");
				...
				jAlert("Direcci&oacute;n encontrada.","Geolocalizaci贸n");
				...
				jAlert("No se pudo geolocalizar la direcci&oacute;n encontrada.","Errores de Geolocalizaci贸n");
